...
void stack_init (void)
{
	...
	//mac auslesen
	if (!datastore_read_value(DATASTORE_ADDRESS_MAC, DATASTORE_ADDRESS_MAC_LENGTH, mymac))
	{
		printInfo("Create new MAC");
		mymac[0] = DEFAULTMAC1;
		mymac[1] = DEFAULTMAC2;
		mymac[2] = DEFAULTMAC3;
		mymac[3] = DEFAULTMAC4;
	...
	for (unsigned char index = 0;index<MAX_TCP_ENTRY;index++)
	{
		if (tcp_entry[index].time == 0)
		{
			if (tcp_entry[index].ip != 0)
			{
				...
				{
					printVerbose("Eintrag wird entfernt MAX_ERROR STACK:%i\r\n",index);
					ETH_INT_DISABLE;
				...
				{
					printVerbose("Packet wird erneut gesendet STACK:%i\r\n",index);
...
void add_tcp_app (unsigned int port, void(*fp1)(unsigned char))
{
	...
	{
		printError("TCP Zuviele Anwendungen wurden gestartet\r\n");
		return;
	}
	printInfo("TCP Anwendung wird in Liste eingetragen: Eintrag %i\r\n",port_index);
...
void change_port_tcp_app (unsigned int port_old, unsigned int port_new)
{
	...
	if (port_index >= MAX_APP_ENTRY)
	{
		printWarning("(Portänderung) Port wurde nicht gefunden\r\n");
		return;
	}
	printInfo("TCP Anwendung Port ändern: Eintrag %i\r\n",port_index);
...
void add_udp_app (unsigned int port, void(*fp1)(unsigned char))
{
	...
	if (port_index >= MAX_APP_ENTRY)
	{
		printWarning("Zuviele UDP Anwendungen wurden gestartet\r\n");
		return;
	}
	printInfo("UDP Anwendung wird in Liste eingetragen: Eintrag %i\r\n",port_index);
                ...
                if(ip->IP_Proto == PROT_ICMP)
                {
                    switch ( icmp->ICMP_Type )
                    {
                        ...
                        case (0): //Ping reply
                            ...
                            printDebug("%i",    (int)((ip->IP_Srcaddr&0x000000FF)    ));
                            printDebug(".%i",   (int)((ip->IP_Srcaddr&0x0000FF00)>>8 ));
                            printDebug(".%i",   (int)((ip->IP_Srcaddr&0x00FF0000)>>16));
                            printDebug(".%i :", (int)((ip->IP_Srcaddr&0xFF000000)>>24));
                            break;
                    }
                    return;
                }
...
void arp_entry_add (void)
{
    ...
    for (unsigned char b = 0; b<MAX_ARP_ENTRY; b++)
    {
        if(arp_entry[b].arp_t_ip == 0)
        {
            ...
            printWarning("Kein ARP oder IP Packet!\r\n");
            return;
        }
    }
    //Eintrag konnte nicht mehr aufgenommen werden
    printWarning("ARP entry tabelle voll!\r\n");
    return;
}
...
void new_eth_header (unsigned char *buffer,unsigned long dest_ip)
{
	...
	printWarning("ARP Eintrag nicht gefunden*\r\n");
	for(a = 0; a < 6; a++)
	{
		//MAC Destadresse wird geschrieben mit MAC Sourceadresse
...
void arp_reply (void)
{
        ...
        arp->ARP_TIPAddr == *((unsigned long*)&myip[0])) // Für uns?
    {
        ...
        {
            arp_entry_add();
            printDebug("ARP REPLY EMPFANGEN!\r\n");
        }
    }
    return;
}
...
char arp_request (unsigned long dest_ip)
{
    ...
    for(count = 0; count<20; count++)
    {
        ...
        {
            printVerbose("ARP EINTRAG GEFUNDEN!\r\n");
        ...
        eth_get_data();
        printWarning("**KEINEN ARP EINTRAG GEFUNDEN**\r\n");
    }
...
void tcp_entry_add (unsigned char *buffer)
{
    ...
    for (unsigned char index = 0;index<(MAX_TCP_ENTRY);index++)
    {
            ...
            (tcp_entry[index].src_port == tcp->TCP_SrcPort)    )
        {
            ...
            printVerbose("TCP Entry gefunden %i\r\n",index);
            return;
        }
    }
    ...
    for (unsigned char index = 0;index<(MAX_TCP_ENTRY);index++)
    {
        if(tcp_entry[index].ip == 0)
        {
            ...
            printVerbose("TCP Entry neuer Eintrag %i\r\n",index);
            return;  
        }
    }
    //Eintrag konnte nicht mehr aufgenommen werden
    printWarning("Server Busy (NO MORE CONNECTIONS)!\r\n");
    return;
}
...
void udp_socket_process(void)
{
	...
	{ 
		...
		printWarning("UDP Keine Anwendung gefunden!\r\n");
		return;
	}
...
void tcp_socket_process(void)
{
	...
	{ 
		//Keine vorhandene Anwendung eingetragen! (ENDE)
		printWarning("TCP Keine Anwendung gefunden!\r\n");
		return;
	}	
	...
	{	
		...
		{
			printWarning("TCP Eintrag nicht erfolgreich!\r\n");
			return;
		}
	
		tcp_entry[index].time = MAX_TCP_PORT_OPEN_TIME;
		printInfo("TCP Port wurde vom Server geöffnet STACK:%i\r\n",index);
	...
	if(tcp->TCP_HdrFlags == SYN_FLAG)
	{
		...
		if (index >= MAX_TCP_ENTRY) //Eintrag gefunden wenn ungleich
		{
			printDebug("TCP Eintrag nicht erfolgreich!\r\n");
			return;
		}
	
		printVerbose("TCP New SERVER Connection! STACK:%i\r\n",index);
	...
	{
		printWarning("TCP Eintrag nicht gefunden\r\n");
		...
		if(tcp->TCP_HdrFlags & FIN_FLAG || tcp->TCP_HdrFlags & RST_FLAG)
		{
			...
			tcp_index_del(index);
			printVerbose("TCP-Stack Eintrag gelöscht! STACK:%i\r\n",index);
			return;
		}
		return;
	}
...
{
	printDebug("Port wird im TCP Stack geschlossen STACK:%i\r\n",index);
...
void tcp_port_open (unsigned long dest_ip,unsigned int port_dst,unsigned int port_src)
{
	...
	ETH_INT_DISABLE;
	printDebug("Oeffen eines Ports mit Server\r\n");
	...
	for (index = 0;index<MAX_TCP_ENTRY;index++)
	{
		if(tcp_entry[index].ip == 0)
		{
			...
			tcp_entry[index].time = MAX_TCP_PORT_OPEN_TIME;
			printDebug("TCP Open neuer Eintrag %i\r\n",index);
			break;
		}
	}
	...
	{
		//Eintrag konnte nicht mehr aufgenommen werden
		printWarning("Busy (NO MORE CONNECTIONS)!\r\n");
	}
